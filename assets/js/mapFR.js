...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const mapEl = document.getElementById('mapFR');
  if (!mapEl) return;
  ...
  const mapFR = L.map('mapFR', mapOptions);
  ...
  fetch('data/cities.json')
    ...
    .then(data => {
      data.forEach(item => {
        ...
        const capLink = item['CAP link'] || item['Cap link'] || item.capLink || item.cap_link || '';

        if (lat && lon) {
          const marker = L.marker([lat, lon]).addTo(mapFR);
          const capLinkHtml = capLink ? `<br/><a href="${capLink}" target="_blank" rel="noopener">Voir le plan d'action climatique</a>` : '';
